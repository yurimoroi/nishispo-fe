...
import { z } from "zod";
import { reporterEditorFormSchema } from "./lib/form-schema";
...
export const FormEdit = ({ currentContent }: Props) => {
  ...
  const form = useForm<z.infer<typeof reporterEditorFormSchema>>({
    mode: "onBlur",
    reValidateMode: "onBlur",
    criteriaMode: "all",
    resolver: zodResolver(reporterEditorFormSchema),
    defaultValues: {
  ...
  const onSubmit = async (
    formData: z.infer<typeof reporterEditorFormSchema>,
  ) => {
    const column = "reporter_editor";
    const pathRedirect = "/admin/companies/work/reporter_editor";
    try {
      setClicked(true);
