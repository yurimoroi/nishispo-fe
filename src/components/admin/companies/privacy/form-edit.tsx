...
import { z } from "zod";
import { privacyFormSchema } from "./lib/form-schema";
...
export const FormEdit = ({ currentContent }: Props) => {
  ...
  const onSubmit = async (formData: z.infer<typeof privacyFormSchema>) => {
    const column = "privacy_policy";
    const pathRedirect = "/admin/companies/privacy";
    try {
      setClicked(true);
