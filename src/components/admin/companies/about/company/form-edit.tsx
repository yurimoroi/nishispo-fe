...
import { z } from "zod";
import { aboutCompanyFormSchema } from "./lib/form-schema";
...
export const FormEdit = ({ currentContent }: Props) => {
  ...
  const onSubmit = async (formData: z.infer<typeof aboutCompanyFormSchema>) => {
    const column = "about_company";
    const pathRedirect = "/admin/companies/about/company";
    try {
      setClicked(true);
