...
import { z } from "zod";
import { companiesAboutFormSchema } from "./lib/form-schema";
...
export const FormEdit = ({ currentContent }: Props) => {
  ...
  const form = useForm<z.infer<typeof companiesAboutFormSchema>>({
    mode: "onBlur",
    reValidateMode: "onBlur",
    criteriaMode: "all",
    resolver: zodResolver(companiesAboutFormSchema),
    defaultValues: {
  ...
  const onSubmit = async (
    formData: z.infer<typeof companiesAboutFormSchema>,
  ) => {
    const column = "about_service";
    const pathRedirect = "/admin/companies/about";
    try {
      setClicked(true);
