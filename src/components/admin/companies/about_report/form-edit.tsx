...
import { z } from "zod";
import { aboutReportFormSchema } from "./lib/form-schema";
...
export const FormEdit = ({ currentContent }: Props) => {
  ...
  const form = useForm<z.infer<typeof aboutReportFormSchema>>({
    mode: "onBlur",
    reValidateMode: "onBlur",
    criteriaMode: "all",
    resolver: zodResolver(aboutReportFormSchema),
    defaultValues: {
      content: `${currentContent}`,
    },
  });

  const onSubmit = async (formData: z.infer<typeof aboutReportFormSchema>) => {
    const column = "about_report";
    const pathRedirect = "/admin/companies/about_report";
    try {
      setClicked(true);
